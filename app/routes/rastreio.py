...
<body>
    ...
    <main class="container py-4">
        <div class="row justify-content-center">
            <div class="col-lg-8">
                <h2 class="text-center mb-4" style="color: #E31E24;">Rastreio de Encomendas</h2>
                <div class="card card-shadow">
                    <div class="card-body">
                        <p class="text-muted text-center">
                            <i class="bi bi-truck display-4 d-block mb-3" style="color: #E31E24;"></i>
                            Digite o código de rastreio da Jadlog para acompanhar sua encomenda.
                        </p>
                        <div class="mt-4">
                            <div class="input-group">
                                <input type="text" class="form-control" placeholder="Ex: 1234567890" id="codigoRastreio">
                                <button class="btn btn-jadlog" onclick="rastrear()">Rastrear</button>
                            </div>
                        </div>
    ...
    <script>
        function rastrear() {
            ...
            resultado.innerHTML = '<div class="text-center"><i class="bi bi-hourglass-split fs-1"></i><p>Buscando rastreio...</p></div>';
            
            // Faz a requisição para a API de rastreio
            fetch(`/rastreio/buscar`, {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                if (data.success) {
                    let html = `<div class="alert alert-success"><strong>${data.status}</strong></div>`;
                    html += `<div class="list-group">`;
                    if (data.historico && data.historico.length > 0) {
                        data.historico.forEach(evento => {
                            html += `<div class="list-group-item">
                                <small class="text-muted">${evento.data || ''} ${evento.hora || ''}</small>
                                <p class="mb-0">${evento.status || 'Evento'}</p>
                            </div>`;
                        });
                    } else {
                        html += `<div class="list-group-item">Nenhum evento encontrado.</div>`;
                    }
                    html += `</div>`;
