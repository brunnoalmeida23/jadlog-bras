...
from app.services.tarifas_capital_v32 import (
    GLM_CAPITAL_PONTOS,
    LUCRO_FAIXAS,
    PRECO_PROMOCIONAL_CAPITAL,
)
from app.services.tarifas_glm_v32 import PESOS_TABELA, TABELAS_GLM
from tests.referencia_cap_int_v32 import REFERENCIA_CAP_INT
...
def test_capital_acima_30_reproduz_ancoras_cap_x_int():
    ...
    for peso in (40, 50, 60, 70, 80, 90, 100):
        ...
        assert res["success"], res
        aud = res["dados"]["_auditoria"]
        assert aud["peso_tabela_glm"] == float(peso)
        assert aud["glm"] == round(GLM_CAPITAL_PONTOS["SP"][peso], 2)


def test_capital_intermediario_usa_proxima_faixa_sem_interpolacao():
    calc = FreteCalculator()
    casos = {
        31: 40,
        35: 40,
    ...
    for peso, faixa in casos.items():
        res = calc.calcular("03309-000", peso, "PACKAGE", 100)
        assert res["success"], res
        aud = res["dados"]["_auditoria"]
        assert aud["peso_tabela_glm"] == float(faixa)
        assert aud["kg_adicionais"] == 0
        assert aud["kg_adicional_unitario"] == 0.0
        assert aud["glm"] == round(GLM_CAPITAL_PONTOS["SP"][faixa], 2)
        assert aud["regra_calculo"] == "CAPITAL_FAIXA_CAP_X_INT_MAIS_LUCRO"


def test_capital_35_sp_bate_com_cap_x_int_corrigida():
    # CAP X INT: SP, faixa até 40 kg -> GLM 105,18 + lucro 140 = 245,18.
    calc = FreteCalculator()
    for modalidade in ("PACKAGE", ".COM"):
        dados = calc.calcular("03309-000", 35, modalidade, 100)["dados"]
        assert dados["preco_final"] == 245.18
        assert dados["_auditoria"]["glm"] == 105.18
        assert dados["_auditoria"]["lucro"] == 140.0


def test_capital_literal_nao_inventa_diferenca_de_modalidade_acima_30():
    # A aba SP CAPITAL não possui colunas PACKAGE/.COM. A modalidade é mantida
    # no resultado, mas não altera esta tabela específica.
    calc = FreteCalculator()
    pack = calc.calcular("03309-000", 35, "PACKAGE", 100)["dados"]
    com = calc.calcular("03309-000", 35, ".COM", 100)["dados"]
    assert pack["preco_final"] == com["preco_final"] == 245.18
    assert pack["modalidade"] == "PACKAGE"
    assert com["modalidade"] == ".COM"
